...
class LaserRobot(SpeedControlledManipulator):
    ...
    def J(self, q):
        ...
        J[0, 0] = (
            self.l[2] * cos_abs[2] + self.l[1] * cos_abs[1] + self.l[0] * cos_abs[0]
        )
        J[0, 1] = self.l[2] * cos_abs[2] + self.l[1] * cos_abs[1]
        J[0, 2] = self.l[2] * cos_abs[2]

        J[1, 0] = (
            -self.l[2] * sin_abs[2] - self.l[1] * sin_abs[1] - self.l[0] * sin_abs[0]
        )
...
class DrillingRobot(ThreeLinkManipulator3D):
    ...
    def H(self, q):
        ...
        H[0, 0] = (
            ...
            + (
                m3
                * (
                    2 * (l2 * c2 * s1 + l3 * c2 * c3 * s1 - l3 * s1 * s2 * s3) ** 2
                    + 2 * (l2 * c1 * c2 + l3 * c1 * c2 * c3 - l3 * c1 * s2 * s3) ** 2
                )
            )
            / 2
            + l2**2 * m2 * c2**2
    ...
    def C(self, q, dq):
        ...
        C[0, 0] = 0
        C[0, 1] = -dq1 * (
            l3**2 * m3 * sin(2 * q2 + 2 * q3)
            + l2**2 * m2 * sin(2 * q2)
            + l2**2 * m3 * sin(2 * q2)
            + 2 * l2 * l3 * m3 * sin(2 * q2 + q3)
        )
        C[0, 2] = (
            -l3
            * m3
            * dq1
            * (l3 * sin(2 * q2 + 2 * q3) + l2 * s3 + l2 * sin(2 * q2 + q3))
        )
...
class DrillingRobotOnJig(DrillingRobot):
    ...
    def forward_kinematic_lines(self, q):
        ...
        lines_pts, lines_style, lines_color = DrillingRobot.forward_kinematic_lines(
            self, q
        )
